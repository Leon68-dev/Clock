...
CClockvcmfcDlg::~CClockvcmfcDlg()
{
	// Signal background threads to stop modifying members
	m_bIsClosing = true;

	if (m_pMapDlg != nullptr)
	{
		if (::IsWindow(m_pMapDlg->GetSafeHwnd()))
		{
			m_pMapDlg->DestroyWindow();
		}
		delete m_pMapDlg;
		m_pMapDlg = nullptr;
	}

	if (m_pWeatherIcon != nullptr)
	{
		delete m_pWeatherIcon;
...
void CClockvcmfcDlg::OnTimer(UINT_PTR nIDEvent)
{
	if (nIDEvent == 1)
	{
		...
		if (st.wSecond != m_lastSecond)
		{
			...
			// Check scheduled shutdown / sleep task
			if (m_isShutDown)
			{
...
void CClockvcmfcDlg::OnMenuShutdown()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		// Persist settings immediately in case the system is shutting down soon
		SaveSettings();
	}
}
...
void CClockvcmfcDlg::SaveSettings()
{
	...
	WriteBool(_T("chk24Hours"), m_b24Hours);
...
{
	if (m_bPingInProgress || !m_bPing || m_strPingAddress.IsEmpty() || m_bIsClosing)
		return;

	m_bPingInProgress = TRUE;

	CString pingAddress = m_strPingAddress;

	std::thread([this, pingAddress]()
		{
			...
			if (GetAddrInfoW(pingAddress, NULL, &hints, &result) == 0 && result != NULL)
			{
			...
			if (ipaddr != INADDR_NONE)
			{
				...
				if (replyBuffer)
				{
					...
					if (dwRetVal != 0)
					{
						...
						pingResult = (int)pEchoReply->RoundTripTime;
					}
					free(replyBuffer);
				}
			}
							...
							do
							{
								...
								if (WinHttpQueryDataAvailable(hRequest, &dwSize) && dwSize > 0)
								{
									...
									if (WinHttpReadData(hRequest, buffer.data(), dwSize, &dwDownloaded))
									{
										...
										jsonResponse += CA2W(buffer.data(), CP_UTF8);
									}
								}
							} while (dwSize > 0);
						}
